...
import { SeHelpdeskFileUploadDialogComponent } from '../se-helpdesk-file-upload-dialog/se-helpdesk-file-upload-dialog.component';

@Component({
  selector: 'app-se-helpdesk-file-upload-details',
  templateUrl: './se-helpdesk-file-upload-details.component.html',
  styleUrls: ['./se-helpdesk-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: SeHelpdeskFileUploadDetailsComponent }]
})
export class SeHelpdeskFileUploadDetailsComponent extends SeHelpdeskProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              protected currencyConverterPipe: CurrencyConverterPipe, protected seHelpdeskProductService: SeHelpdeskProductService) {
    ...
    this.module = this.translateService.instant('seHelpdeskFileUploadDetails');
  }
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('seHelpdeskFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('seHelpdeskFileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'seHelpdeskFileUploadDetails';
      let bankType;
  ...
  onClickBrowseButton() {
    ...
    const dialogRef = this.dialogService.open(SeHelpdeskFileUploadDialogComponent, {
  ...
  ngOnDestroy() {
    ...
    // }
    this.stateService.setStateSection('seHelpdeskFileUploadDetails', this.form);
  }

  onClickNext() {
    this.saveFormObject();
    this.saveDraftService.changeSaveStatus('seHelpdeskFileUploadDetails',
    this.stateService.getSectionData('seHelpdeskFileUploadDetails'));
  }
