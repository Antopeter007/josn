...
import { CardSummaryComponent } from './../../../../../../../shared/FCCform/form-controls/modules/card-summary/card-summary.component';
import { ExpansionPanelEditFormTableComponent } from './../../../../../../../shared/FCCform/form-controls/modules/expansion-panel-edit-form-table/expansion-panel-edit-form-table.component';
...
import { POColumnsHelper } from '../../../model/poColumnsHelper';
...
import { PurchaseOrderLineItemMap } from '../../../services/purchaseOrderLineItem';
...
@Component({
  ...
  providers: [{ provide: HOST_COMPONENT, useExisting: DescriptionOfGoodsComponent }, ExpansionPanelEditFormTableComponent,
...
export class DescriptionOfGoodsComponent extends LcProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected emitterService: EventEmitterService, protected stateService: ProductStateService,
    ...
    public expansionPanelEditFormTableComponent: ExpansionPanelEditFormTableComponent,
    public cardSummary: CardSummaryComponent,
    ...
    protected currencyConverterPipe: CurrencyConverterPipe, protected lcProductService: LcProductService, protected sanitizer: DomSanitizer,
  ...
  ngOnInit(): void {
    ...
    if (this.stateService.getSectionData('amountChargeDetails').get('currency').value !== null &&
      ...
      const currency = this.stateService.getSectionData('amountChargeDetails').get('currency').value.currencyCode;
      ...
      if (this.form.get(FccGlobalConstant.LINEITEM1) && this.form.get(FccGlobalConstant.LINEITEM1).value) {
        ...
        if(this.form.get('purchaseOrderToggle').value === 'Y' && this.purchaseOrders && this.purchaseOrders.length>0){
  ...
  onClickDescOfGoodsText(){
    this.onBlurDescOfGoodsText();
  }

  onBlurDescOfGoodsText(){
    const data = this.form.get(this.element).value;
    const checkForValue = this.commonService.isNonEmptyValue(data) ? data.replace( /[\r\n]+/gm, '' ) : '';
    if (checkForValue === '') {
      this.form.get(this.element).setValue('');
  ...
  onClickPurchaseOrderToggle() {
    ...
    if (togglevalue === FccBusinessConstantsService.NO) {
      ...
      if (this.stateService.getSectionData('amountChargeDetails').get('currency').value !== null &&
        ...
        this.form.get('purchaseOrderToggle')[this.params]['warning'] = `${this.translateService.instant(this.mutualExclusiveMessage)}`;
      }
    }
  }
  ...
  onKeyupDeductions(){
    ...
    if(this.form.get('deductions').value && !isNaN(this.form.get('deductions').value)){
      if(Number(this.form.get('deductions').value) > Number(TotalGrossAmount)){
        const narrativeDetailsForm = this.stateService.getSectionData('narrativeDetails');
        if(narrativeDetailsForm !== undefined){
          const goodsandDocForm = narrativeDetailsForm.get('goodsandDoc');
          if(goodsandDocForm !== undefined){
            const descOfGoodsForm = goodsandDocForm.get('descOfGoods');
  ...
  private caleculateLCAmount(TotalGrossAmount: number) {
    let lcAmount=0;
    const currency = this.stateService.getSectionData('amountChargeDetails').get('currency').value.currencyCode;
    if (this.form.get('goodsPercentageValue').value) {
  ...
  onClickNewPurchaseButton() {
    ...
    // commenting since 'currency' is assigned a value but never used
    // const currency = this.stateService.getSectionData('amountChargeDetails').get('currency').value.currencyCode;
    this.form.get('deductions')[this.params][FccGlobalConstant.RENDERED] = true;
    this.form.get('poSeparator')[this.params][FccGlobalConstant.RENDERED] = true;
    this.form.get('poOverrideError')[this.params][FccGlobalConstant.RENDERED] = true;
  ...
  constuctLineItemObj() {
    if (this.form
      ...
      if (typeof lineItem === 'string' && lineItem && lineItem !== undefined && lineItem !== '') {
  ...
  onClickOverrideLCAmount(obj: any) {
    if (obj.value === 'overrideLCAmountYes') {
      ...
      this.stateService.setValue('amountChargeDetails', 'amount', totalGrossAmountVal.toString(), false);
      this.form.get('grossAmountError')[this.params][FccGlobalConstant.RENDERED] = false;
    ...
    const amountChargeDetailsForm = this.stateService.getSectionData('amountChargeDetails');
  ...
  formatTableHeaderValues(event: any, key: any, product: any) {
    const currency = this.stateService.getSectionData('amountChargeDetails').get('currency').value.shortName;
  ...
  setTotalGrossAmount(totalGrossAmount, from){
    ...
    if (this.stateService.getSectionData('amountChargeDetails').get('amount') &&
     (totalGrossAmount !== '0' && totalGrossAmount!== '0.00' && totalGrossAmount !== 0)
     && Number(this.stateService.getSectionData('amountChargeDetails').get('amount').value) !== Number(totalGrossAmount)
  ...
  updateDataArray() {
    ...
    this.stateService.getSectionData('generalDetails').get('purchaseOrderReference').setValue(poReferences);
    this.form.updateValueAndValidity();
  }
  ...
  buildTable() {
    ...
    this.purchaseOrders.forEach(product => {
      ...
      if (product.PurchaseOrderID) {
        const nbDec = this.stateService.getSectionData('amountChargeDetails').get('currency').value.currencyCode;
