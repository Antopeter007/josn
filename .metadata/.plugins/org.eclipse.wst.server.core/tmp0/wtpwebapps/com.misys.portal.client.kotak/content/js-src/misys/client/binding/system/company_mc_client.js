 ...
 * @param m
 */
var transactionBoeJSONArray = [];
var transactionDataAddressArray = [];
var transactionAddJSONArray = [];
var jsonDataAddress = {};
(function(/*Dojo*/d, /*Dijit*/dj, /*Misys*/m) {
	...
	d.mixin(m, {
		...
		bind: function() {
			...
			m.setValidation("new_country_name", m.validateCountry);
		...
		beforeSubmitValidations: function() {
			...
			console.log(dj.byId("company_id").get("value"));

			/* console.log(dj.byId("transaction_address_json_data").get("value"));
				  if(dj.byId("transaction_address_json_data")){
				transactionAddJSONArray = JSON.parse(dj.byId("transaction_address_json_data").get("value"));
				}	 
			 if(transactionAddJSONArray != null){
			 for(var i = 0; i < transactionAddJSONArray.length; i++) {
			 delete transactionAddJSONArray[i]['action'];
			 }
			 console.log("transactionAddJSONArray " + transactionAddJSONArray);
			 dj.byId("transaction_address_json_data").set("value", JSON.stringify(transactionAddJSONArray));
			 }
			 console.log("transactionAddJSONArray " + dj.byId("transaction_address_json_data").get("value"));
*/
	...
	d.mixin(m, {
		...
		_autoAddAddressOpen: function() {
			dj.byId("importaddressDialog").show();
			dj.byId("add_id").set("value", "");
			dj.byId("other_add_line_1").set("value", "");
			dj.byId("other_add_line_2").set("value", "");
			dj.byId("other_add_line_3").set("value", "");
			dj.byId("new_country_name").set("value", "");
		},

		_validateRequired: function() {


			var dialogRequiredFields = ["other_add_line_1", "new_country_name"];
			var isStateValid = true;
			for (var field in dialogRequiredFields) {
				var fieldname = dialogRequiredFields[field];
				if (dijit.byId(fieldname) && (dijit.byId(fieldname).getValue() == null || dijit.byId(fieldname).getValue() == "" || dijit.byId(fieldname).getValue() + "" == "NaN" || dijit.byId(fieldname).state === 'Error')) {
		...
		_addNewAddress: function() {
			...
			var validateRequiredStaus = this._validateRequired();
			if (!validateRequiredStaus) {
				var displayMessage = "Please fill mandatory";
				m.dialog.show("ERROR", displayMessage);
				return false;
			}

			var jsonArray = dj.byId("transaction_address_json_data").get("value");
			console.log("transaction_address_json_data " + jsonArray);
			var jsonArray;
			var addId = dj.byId("add_id").get("value");
			var addrLine1 = dj.byId("other_add_line_1").get("value");

			var addrLine2 = dj.byId("other_add_line_2").get("value");
			var addrLine3 = dj.byId("other_add_line_3").get("value");
			var addCtry = dj.byId("new_country_name").get("value");
			var i;
			if (addId) {
				var ormObj;
				if (transactionBoeJSONArray.length > 0) {
					for (i = 0; i < transactionBoeJSONArray.length; i++) {
						var orm_id;
						if (d.isArray(transactionBoeJSONArray[i]["id"])) {
							orm_id = transactionBoeJSONArray[i]["id"][0];
						}
						else {
							orm_id = transactionBoeJSONArray[i]["id"];
						}

						if (parseInt(orm_id) == parseInt(addId)) {
							ormObj = transactionBoeJSONArray[i];
				...
				transactionBoeJSONArray[i]["add_id"] = addId;
				transactionBoeJSONArray[i]["other_add_line_1"] = addrLine1;
				transactionBoeJSONArray[i]["other_add_line_2"] = addrLine2;

				transactionBoeJSONArray[i]["other_add_line_3"] = addrLine3;
				transactionBoeJSONArray[i]["new_country_name"] = addCtry;



				transactionAddJSONArray = dj.byId("transaction_address_json_data").get("value");
				transactionAddJSONArray = JSON.parse(transactionAddJSONArray);
				transactionAddJSONArray[i]["add_id"] = addId;
				transactionAddJSONArray[i]["other_add_line_1"] = addrLine1;
				transactionAddJSONArray[i]["other_add_line_2"] = addrLine2;
				transactionAddJSONArray[i]["other_add_line_3"] = addrLine3;
				transactionAddJSONArray[i]["new_country_name"] = addCtry;


				dj.byId("transaction_address_json_data").set("value", JSON.stringify(transactionAddJSONArray))


				m._setMultipleAddressGrid(transactionBoeJSONArray);
			} else {

				var transactionObj = {};
				transactionObj["add_id"] = addId;
				transactionObj["other_add_line_1"] = addrLine1;

				transactionObj["other_add_line_2"] = addrLine2;
				//m._formatAmount(amtLiq, tranAmt);	
				transactionObj["other_add_line_3"] = addrLine3;
				transactionObj["new_country_name"] = addCtry;



				if (!jsonArray.length) {
					transactionBoeJSONArray.push(transactionObj);

					dj.byId("transaction_address_json_data").set("value", JSON.stringify(transactionBoeJSONArray))


					transactionAddJSONArray.push(transactionObj);

					m._setMultipleAddressGrid(transactionBoeJSONArray);

				} else {

					transactionBoeJSONArray = dj.byId("transaction_address_json_data").get("value");
					transactionAddJSONArray = dj.byId("transaction_address_json_data").get("value");

					transactionBoeJSONArray = JSON.parse(transactionBoeJSONArray);
					transactionAddJSONArray = JSON.parse(transactionAddJSONArray);
					transactionBoeJSONArray.push(transactionObj);
			...
			dj.byId("importaddressDialog").hide();

			var dispJsonData = dj.byId("transaction_address_json_data").get("value");
			dispJsonData = JSON.parse(dispJsonData);
			var data = {
				identifier: 'id',
				label: 'id',
				items: []
			...
			d.forEach(dispJsonData, function(dispJsonId, index) {



				dispJsonId["id"] = index + 1;

			});

			console.log("dispJsonId " + dispJsonData);
			dj.byId("transaction_address_json_data").set("value", JSON.stringify(dispJsonData))
		...
		_setMultipleAddressGrid: function(transactionBoeJSONArray) {

			var layout = [];

			console.log("transactionBoeJSONArray " + transactionBoeJSONArray);
			if (transactionBoeJSONArray && transactionBoeJSONArray.length == 0 || transactionBoeJSONArray == undefined) {
				document.getElementById("AddressMultipleGridContainer").innerHTML = "No records found";
				return;
			}
			...
			d.forEach(transactionBoeJSONArray, function(singleTransactionBoeJSONArray, index) {

				var editButton;
				var deleteButton;
				var displayButton = "";

				singleTransactionBoeJSONArray["id"] = index + 1;


				editButton = "<a style='cursor: pointer;' onclick='javascript:misys._editAddrsTransaction(" + singleTransactionBoeJSONArray["id"] + ")'><img src='" + misys.getContextualURL(misys._config.imagesSrc +
					m._config.imageStore.editIcon) + "' alt='Edit'></a>";

				displayButton = editButton;


				deleteButton = "<a style='cursor: pointer;' onclick='javascript:misys._deleteAddressTransaction(" + singleTransactionBoeJSONArray["id"] + ")'><img src='" + misys.getContextualURL(misys._config.imagesSrc +
					m._config.imageStore.deleteIcon) + "' alt='Delete'></a>";

				singleTransactionBoeJSONArray["action"] = displayButton + deleteButton;
			});

			data.items = transactionBoeJSONArray;
			...
			var layout = [{
				...
				name: "Address line 1",
				field: "other_add_line_1",
				width: "10%",
				styles: 'text-align: center;'
			},
			{
				name: "Address line 2",
				field: "other_add_line_2",
				width: "10%",
				styles: 'text-align: center;'
			},
			{
				name: "Address line 3",
				field: "other_add_line_3",
				width: "10%",
				styles: 'text-align: center;'
			},
			{
			...
			document.getElementById("AddressMultipleGridContainer").innerHTML = "";
			d.byId('AddressMultipleGridContainer').appendChild(grid.domNode);
			grid.startup();
			grid.setStore(store);
		},


		_setMultipleAddressGridView: function(transactionBoeJSONArray) {
		...
		_deleteAddressTransaction: function(id) {
			if (!transactionBoeJSONArray.length) {

				transactionBoeJSONArray = dj.byId("transaction_address_json_data").get("value");
				transactionBoeJSONArray = JSON.parse(transactionBoeJSONArray);
			}
			if (transactionBoeJSONArray.length > 0) {
				for (var i = 0; i < transactionBoeJSONArray.length; i++) {
					if (parseInt(transactionBoeJSONArray[i]["id"]) == id) {
						transactionBoeJSONArray.splice(i, 1);
						transactionAddJSONArray.splice(i, 1);
						break;
					}
				}
			}
			console.log("gridIRMFIRCMappedJson " + transactionAddJSONArray);
			dj.byId("transaction_address_json_data").set("value", JSON.stringify(transactionBoeJSONArray))
		...
		_hideTransactionAddons: function(gridId) {
			dj.byId("importaddressDialog").hide();
		},


		_editAddrsTransaction: function(id) {
			...
			var transactionBoeObj;
			if (transactionBoeJSONArray.length > 0) {
				for (var i = 0; i < transactionBoeJSONArray.length; i++) {
					if (parseInt(transactionBoeJSONArray[i]["id"]) == id) {
						transactionBoeObj = transactionBoeJSONArray[i];
						break;
					}
				}
			}
			var addId = transactionBoeObj["id"];
			var addrLine1 = transactionBoeObj["other_add_line_1"];

			var addrLine2 = transactionBoeObj["other_add_line_2"];
			var addrLine3 = transactionBoeObj["other_add_line_3"];
			var addCtry = transactionBoeObj["new_country_name"];

			dj.byId("add_id").set("value", addId);
			dj.byId("other_add_line_1").set("value", addrLine1);
			dj.byId("other_add_line_2").set("value", addrLine2);
			dj.byId("other_add_line_3").set("value", addrLine3);
			dj.byId("new_country_name").set("value", addCtry);



			m.showTransactionAddonsDialog('importaddress');
		},
		...
		m.xhrPost({
			...
			load: function(response, args) {
				...
				if (data != "" && data.status == "SUCCESS") {
					...
					var bcifFetch = dj.byId("bcif_fetch_company_status");
	...
	function setCorporateDetails(data) {
		if (data.response.Response) {

			var bcifFetch = dj.byId("bcif_fetch_company_status");
			bcifFetch.set("value", true);

            var corporateName = dj.byId('name');
			if (corporateName) {
				if (data.response.Response.party_demographic.first_name != "" 
				|| data.response.Response.party_demographic.middle_name != "" 
				|| data.response.Response.party_demographic.last_name != "" ) {
				
					var corporateNameValue1 = data.response.Response.party_demographic.first_name;
					var corporateNameValue2 = data.response.Response.party_demographic.middle_name;
					var corporateNameValue3 = data.response.Response.party_demographic.last_name;
					
					var corporateNameValue = corporateNameValue1.concat(corporateNameValue2 ,corporateNameValue3);
					
					console.log("corporateNameValue --->"+corporateNameValue);
					
					corporateName.set("value", corporateNameValue);
				}

			}
			
			
			var country = dj.byId('_country');
			if (country) {
			...
			var townName = dj.byId('_town_name');
			if (townName) {
				if (data.response.Response.party_address.city != "") {
					var townNameValue = data.response.Response.party_address.city;
					townName.set("value", townNameValue);
				}

			}
			var postCode = dj.byId('_post_code');
			if (postCode) {
			...
			var countrySubDivision = dj.byId('_country_sub_div');
			if (countrySubDivision) {
				if (data.response.Response.party_address.state != "") {
					var countrySubDivValue = data.response.Response.party_address.state;
					countrySubDivision.set("value", countrySubDivValue);
				}

			}
			var phone = dj.byId('phone');
			if (phone) {
				if (data.response.Response.contact_details.pref_mobile != "") {
					var phoneValue = data.response.Response.contact_details.pref_mobile;
			...
			if (data.response.Response.party_address) {
			
			
			var addressLine1 = dj.byId('address_line_1');
			if (addressLine1) {
				if (data.response.Response.party_address.address1 != "") {
					var addressLine1Value = data.response.Response.party_address.address1;
					addressLine1.set("value", addressLine1Value);
				}

			}
			var addressLine2 = dj.byId('address_line_2');
			if (addressLine2) {
				if (data.response.Response.party_address.address2 != "") {
					var addressLine2Value = data.response.Response.party_address.address2;
					addressLine2.set("value", addressLine2Value);
				}
			...
			if (dom) {
				if (data.response.Response.party_address.address3 != "") {
					var domValue = data.response.Response.party_address.address3;
				...
				var transactionObj = {};
				transactionObj["add_id"] = "";
				transactionObj["other_add_line_1"] = data.response.Response.party_address.address1;
				transactionObj["other_add_line_2"] = data.response.Response.party_address.address2;
				transactionObj["other_add_line_3"] = data.response.Response.party_address.address3;
				transactionObj["new_country_name"] = "IN";

				dj.byId('add_id').set("value", "");
				dj.byId('other_add_line_1').set("value", data.response.Response.party_address.address1);
				dj.byId('other_add_line_2').set("value", data.response.Response.party_address.address2);
				dj.byId('other_add_line_3').set("value", data.response.Response.party_address.address3);
				dj.byId('new_country_name').set("value", "IN");

				misys._addNewAddress();
	...
	d.mixin(m, {
		...
		client: {
			...
			beforeSubmitValidations: function() {
				var bcifFetch = dj.byId("bcif_fetch_company_status");
				var bcifCompanyType = dj.byId("type");
