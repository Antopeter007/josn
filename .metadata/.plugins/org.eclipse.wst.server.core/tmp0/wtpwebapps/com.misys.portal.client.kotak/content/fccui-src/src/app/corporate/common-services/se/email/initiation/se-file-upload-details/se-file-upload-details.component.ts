...
import { SeFileUploadDialogComponent } from '../se-file-upload-dialog/se-file-upload-dialog.component';

@Component({
  selector: 'app-se-file-upload-details',
  templateUrl: './se-file-upload-details.component.html',
  styleUrls: ['./se-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: SeFileUploadDetailsComponent }]
})
export class SeFileUploadDetailsComponent extends SeProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              protected currencyConverterPipe: CurrencyConverterPipe, protected seProductService: SeProductService) {
    ...
    this.module = this.translateService.instant('seFileUploadDetails');
  }
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData('seFileUploadDetails').get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData('seFileUploadDetails')
    ...
    initializeFormGroup() {
      const sectionName = 'seFileUploadDetails';
      let bankType;
      this.form = this.stateService.getSectionData(sectionName);
      if (this.tnxTypeCode === FccGlobalConstant.N002_INQUIRE) {
        bankType = this.stateService.getValue(FccGlobalConstant.SE_GENERAL_DETAILS,
          ...
          await this.autoUploadFile.uploadAttachments(this.fileModel.file, this.fileModel.title, lcNumber, eventId).toPromise().then(
            response => {
              ...
              if (error.status === FccGlobalConstant.HTTP_RESPONSE_BAD_REQUEST
                ...
                if (error.error && (error.error.detail && error.error.detail.indexOf(FccGlobalConstant.MALICIOUS) > -1)
  ...
  onClickBrowseButton() {
    ...
    const dialogRef = this.dialogService.open(SeFileUploadDialogComponent, {
  ...
  ngOnDestroy() {
    this.stateService.setStateSection('seFileUploadDetails', this.form);
  }

  onClickNext() {
    this.saveFormObject();
    this.saveDraftService.changeSaveStatus('seFileUploadDetails',
    this.stateService.getSectionData('seFileUploadDetails'));
  }
