...
import { SeUploadFileUploadDialogComponent } from '../se-upload-file-upload-dialog/se-upload-file-upload-dialog.component';
import { UntypedFormControl, Validators } from '@angular/forms';

@Component({
  selector: 'app-se-upload-file-upload-details',
  templateUrl: './se-upload-file-upload-details.component.html',
  styleUrls: ['./se-upload-file-upload-details.component.scss'],
  providers: [{ provide: HOST_COMPONENT, useExisting: SeUploadFileUploadDetailsComponent }]
})
export class SeUploadFileUploadDetailsComponent extends SeUploadProductComponent implements OnInit, OnDestroy {
  ...
  constructor(protected commonService: CommonService, protected sessionValidation: SessionValidateService,
              ...
              protected currencyConverterPipe: CurrencyConverterPipe, protected seUploadProductService: SeUploadProductService) {
    ...
    this.module = this.translateService.instant(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS);
  }
  ...
  ngOnInit() {
    ...
    if (this.stateService.getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS)
      .get('attachments').value[FccGlobalConstant.DOC_ID]) {
      this.renderBrowseButton(this.stateService.getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS)
    ...
    if (this.stateService.getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS)
    .get('noOfAttachments').value > 0) {
      this.form.get('attachmentsMessage')[FccGlobalConstant.PARAMS][this.rendered] = false;
      this.form.get('attachmentHidden').setValue(this.stateService
        .getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS)
      .get('noOfAttachments')?.value);
    } else {
    ...
    initializeFormGroup() {
      const sectionName = FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS;
      let bankType;
      this.form = this.stateService.getSectionData(sectionName);
      if (this.tnxTypeCode === FccGlobalConstant.N002_INQUIRE) {
        bankType = this.stateService.getValue(FccGlobalConstant.SE_UPLOAD_GENERAL_DETAILS,
  ...
  async onAutoUploadHandler(event, uploader: FileUpload) {
    ...
    for (let i = 0; i < filesLength; i++) {
      if (this.uploadFile.numberOfFiles < this.numberOfFilesRegex) {
        ...
        this.fileModel = new FileMap(filesArray[i], filesArray[i].name, filesArray[i].name, this.getFileExt(filesArray[i].name),
          this.getFileExtPath(filesArray[i].name), this.getFileSize(filesArray[i].size), null, null, null, null, null);
            if (this.isFileSizeValid && this.isFileExtValid && this.checkValidFileNameRegex(event)) {
              ...
              await this.autoUploadFile.uploadAttachments(this.fileModel.file, this.fileModel.title,
                lcNumber, eventId, null, 'MD5').toPromise().then(
            response => {
        ...
        } else if (!this.isFileNameCharValid) {
          this.messageSet.add(FccGlobalConstant.LENGTH_5);
        }
      } else {
        this.messageSet.add(0);
  ...
  onClickBrowseButton() {
    ...
    const dialogRef = this.dialogService.open(SeUploadFileUploadDialogComponent, {
  ...
  onClickTrashIcon(event, key, index) {
    ...
    dialogRef.onClose.subscribe((result: any) => {
      if (result.toLowerCase() === 'yes') {
        ...
        this.form.controls.attachments.value.docId.splice(0);
        if (this.form.get('noOfAttachments') === null || this.form.get('noOfAttachments') === undefined) {
          this.form.addControl('noOfAttachments', new UntypedFormControl (this.fileListSvc.fileMap.length));
        }
        this.noOfAttachments = this.form.get('noOfAttachments').value;
        this.form.get('noOfAttachments').setValue(--this.noOfAttachments);
        this.form.get('noOfAttachments').updateValueAndValidity();
  ...
  showMessage() {
    if (this.messageSet.has(0)) {
      ...
      this.patchFieldParameters(this.form.get('fileUploadTable'), { message: `${this.translateService.instant('maliciousContentError')}` });
    } else if (this.messageSet.has(FccGlobalConstant.LENGTH_6)) {
      this.patchFieldParameters(this.form.get('fileUploadTable'), {
  ...
  saveFormObject() {
    this.stateService.setStateSection(
      FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS,
      this.form
    );
  }

  checkAttachments() {
    const fileLength = this.stateService.getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS)
      .get('attachments')?.value[FccGlobalConstant.DOC_ID]?.length;
    if ((fileLength === FccGlobalConstant.ZERO || fileLength === null)) {
      ...
      this.form.get(FccGlobalConstant.ATTACHMENTS).setErrors({ required: true });
  ...
  ngOnDestroy() {
    this.checkAttachments();
    this.stateService.setStateSection(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS, this.form);
  }

  onClickNext() {
    this.saveFormObject();
    this.saveDraftService.changeSaveStatus(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS,
    this.stateService.getSectionData(FccGlobalConstant.SE_UPLOAD_FILE_UPLOAD_DETAILS));
  }
