...
=====*/
	...
	_addItem: function(item, index, noUpdate){
		this._by_idx[index] = this._createItem(this._sanitizeHTMLDataForUI(item), index);
		if(!noUpdate){
			this.updateRow(index);
		}
	},
	
	_sanitizeHTMLDataForUI : function( /*RowItem*/ item) {
		if(item){
			var propertyKeys = Object.keys(item);
			propertyKeys.forEach(function (property, index){
				if((property.includes('key_') || property.includes('data_') || property.includes('KEY_') || property.includes('DATA_')) && 
					item[property] && 
					item[property][0]
				) {
					item[property][0] = item[property][0].replace(/</g, "&lt;");
					item[property][0] = item[property][0].replace(/>/g, "&gt;");	
				}
			});	
		}
